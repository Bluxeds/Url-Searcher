...
class UrlSearcher:
    def __init__(self, url: str, path_to_keywords: str) -> None:
        try:
            ...
            self._search_keywords: list[str] = (
                []
            )  # Holds all the keywords to add to the end of the url
            self._endpoint_keyword_urls: list[str] = (
                []
            )  # Holds all the urls with a keyword on the endpoint
    ...
    def search_url_endpoints(self):
        ...
        for url in self._endpoint_keyword_urls:
            ...
            print(
                f"[{counter}/{len(self._endpoint_keyword_urls)}] Endpoints Searched | Valid Urls Found: {valid_url_counter}"
            )
            ...
            if counter == len(self._endpoint_keyword_urls):
                ...
                if valid_url_counter == 0:
                    ...
                    for number, url in enumerate(
                        non_duplicate_valid_urls_list, start=1
                    ):
